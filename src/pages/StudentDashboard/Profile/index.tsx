...
const Profile: React.FC = () => {
    ...
    const [day, setDay] = useState<number>(user ? user?.birthDay[2] : 1);
    const [month, setMonth] = useState<number>(user ? user.birthDay[1] : 1);
    const [year, setYear] = useState<number>(user ? user.birthDay[0] : 2000);
    ...
    const maxDayofmonth = (month: number, year: number): number => {
        if (month === 2) {
